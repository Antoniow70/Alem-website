    ...
    <div className="bg-slate-50 min-h-screen pb-24">
      ...
      <section className="bg-gradient-to-br from-brand-primary-dark via-brand-primary to-slate-950 text-white py-28 px-4 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto text-center space-y-6 relative z-10">
          <h1 className="text-5xl font-bold tracking-tight">Projetos Sociais</h1>
          <p className="text-xl text-blue-100/80 max-w-2xl mx-auto leading-relaxed">
            Conheça as nossas iniciativas em curso e o impacto que estamos a gerar nas comunidades.
          </p>
        </div>
      </section>
        ...
        <div className="bg-amber-50 border-b border-amber-200 py-3 px-4">
          <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 text-amber-800 text-sm font-medium">
              ...
              <p>
                <span className="font-bold">Modo de Demonstração:</span> O Supabase não está configurado. Os dados abaixo são fictícios.
              </p>
            </div>
            <a 
              href="/admin" 
      ...
      {/* Filters */}
      <section className="py-12 px-4 sticky top-16 z-40 bg-slate-50/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-6 items-center justify-between">
          <div className="flex bg-white p-1 rounded-2xl shadow-sm border border-slate-200 w-full md:w-auto overflow-x-auto">
            {['Todos', 'Planeamento', 'Em Curso', 'Concluído'].map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-6 py-2 rounded-xl text-sm font-bold transition-all whitespace-nowrap ${
                  filter === f ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-900'
                }`}
              >
                {f}
              </button>
        ...
        {loading ? (
          ...
          <div className="text-center py-32 space-y-6 bg-white rounded-[40px] border border-dashed border-slate-200">
            ...
            <div className="space-y-2">
              <h3 className="text-xl font-bold text-slate-900">Nenhum projeto encontrado</h3>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-md ${
                      selectedProject.status === 'Planeamento' ? 'bg-blue-100 text-blue-700' :
                      selectedProject.status === 'Em Curso' ? 'bg-green-100 text-green-700' :
              ...
              <div className="overflow-y-auto p-4 md:p-12 space-y-12">
                ...
                <div className="rounded-[32px] overflow-hidden bg-slate-100 max-h-[70vh] relative shadow-inner flex items-center justify-center">
                  {selectedProject.media_type === 'image' ? (
                    <img 
                      ...
                      alt={selectedProject.media_desc || selectedProject.name}
                    ...
                    <div className="w-full aspect-video">
                      <iframe
                        src={`https://www.youtube.com/embed/${getYouTubeId(selectedProject.media_url)}`}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                  <div className="lg:col-span-2 space-y-8">
                    <div className="space-y-4">
                      <h3 className="text-3xl font-bold text-slate-900">Sobre o Projeto</h3>
                    ...
                    {selectedProject.gallery && selectedProject.gallery.length > 0 && (
                      <div className="space-y-6 pt-8 border-t border-slate-100">
                        <h3 className="text-2xl font-bold text-slate-900">Galeria do Projeto</h3>
                            ...
                            <div key={idx} className="space-y-2 group">
                              <div className="rounded-2xl overflow-hidden bg-slate-100 aspect-video relative shadow-sm">
                                {item.type === 'image' ? (
                                  <img 
                                    src={item.url} 
                                    alt={item.description || `Média ${idx + 1}`}
                  ...
                  <div className="space-y-8">
                    <div className="bg-slate-50 p-8 rounded-[32px] space-y-6">
                      <h4 className="font-bold text-slate-900 uppercase text-xs tracking-widest">Detalhes</h4>
                      <div className="space-y-4">
                        <div className="flex items-center gap-3 text-slate-600">
                          <Calendar size={18} className="text-blue-600" />
                          <div className="text-sm">
                            <p className="font-bold text-slate-900">Data de Registo</p>
                            <p>{new Date(selectedProject.created_at).toLocaleDateString('pt-PT')}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3 text-slate-600">
                          <Tag size={18} className="text-blue-600" />
                          <div className="text-sm">
                            <p className="font-bold text-slate-900">Estado Atual</p>
                ...
                <div className="space-y-8 pt-12 border-t border-slate-100">
                  <div className="flex justify-between items-end">
                    <h3 className="text-2xl font-bold text-slate-900">Outros Projetos</h3>
                    <button 
                      onClick={() => setSelectedProject(null)}
                      ...
                      .map(p => (
                        ...
                        >
                          <div className="w-24 h-24 rounded-2xl overflow-hidden shrink-0">
                            <img 
                              src={p.media_type === 'image' ? p.media_url : `https://img.youtube.com/vi/${getYouTubeId(p.media_url)}/default.jpg`}
                              alt=""
