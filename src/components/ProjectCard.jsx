...
export default function ProjectCard({ project, onClick }) {
  const statusColors = {
    'Planeamento': 'bg-amber-500/90 text-white',
    'Em Curso': 'bg-green-500/90 text-white',
    'Concluído': 'bg-slate-500/90 text-white',
  };
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      onClick={onClick}
      className="relative rounded-[28px] overflow-hidden group cursor-pointer aspect-[4/5] shadow-lg hover:shadow-2xl transition-all duration-500"
    >
      {/* Full-bleed image */}
      {project.media_type === 'image' ? (
        <img
          src={project.media_url}
          alt={project.media_desc || project.name}
        ...
        <>
          <img
            src={`https://img.youtube.com/vi/${getYouTubeId(project.media_url)}/hqdefault.jpg`}
            alt={project.name}
            className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-out"
            referrerPolicy="no-referrer"
          />
      ...
      <div className="absolute top-5 right-5 bg-white/10 backdrop-blur-md p-2 rounded-full text-white z-10">
        {project.media_type === 'image' ? <ImageIcon size={14} /> : <Play size={14} />}
      </div>
